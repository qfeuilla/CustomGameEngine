...
{
public:
	static IndexedTriangleList MakeTesselated(dynamical::VertexLayout layout, int latDiv, int longDiv)
	{
		...
		dynamical::VertexBuffer vb{ std::move(layout) };
	...
	static IndexedTriangleList Make(std::optional<dynamical::VertexLayout> layout = std::nullopt)
	{
		using Element = dynamical::VertexLayout::ElementType;
		if (!layout)
		{
			layout = dynamical::VertexLayout{}.Append(Element::Position3D);
		}
